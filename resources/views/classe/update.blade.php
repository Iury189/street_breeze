        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ url("update-classe/$classe->id") }}" method="POST">
                        @csrf @method('PATCH')
                        <div>
                            <x-input-label for="id_fighter" :value="__('Fighter:')" />
                            <x-select id="id_fighter" class="block w-full" name="id_fighter" autofocus>
                                <option {{ $classe->id_fighter == '' ? 'selected' : '' }} value="">{{ __('Escolha o fighter') }}</option>
                                @foreach($fighter as $f)
                                    <option {{ $classe->id_fighter == $f->id ? 'selected' : '' }} value="{{ $f->id }}">{{ $f->nome }}</option>
                                @endforeach
                            </x-select>
                        </div>
                        <br>
                        <div>
                            ...
                            <x-select id="id_master" class="block w-full" name="id_master" autofocus>
                                <option {{ $classe->id_master == '' ? 'selected' : '' }} value="">{{ __('Escolha o master') }}</option>
                                @foreach($master as $m)
                                    <option {{ $classe->id_master == $m->id ? 'selected' : '' }} value="{{ $m->id }}">{{ $m->nome }}</option>
                                @endforeach
                            </x-select>
                        </div>
                        <br>
